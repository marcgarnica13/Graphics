...
{   
    ...
    vs.compileSourceCode("#version 330 core \n layout (location = 0) in vec3 vertex; layout (location = 1) in vec3 normal; uniform mat4 modelViewProjectionMatrix; out vec4 frontColor;    void main() { frontColor = vec4(normal,1); gl_Position = modelViewProjectionMatrix * vec4(vertex, 1.0); }");
