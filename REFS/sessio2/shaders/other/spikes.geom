...
void main( void )
{
	...
	vec4 newP = bari + vec4(disp*N,0);
	
	// the definition of vertices is cylic
	for (int i = 0; i < 3; i++) {		
		vec3 a = gl_PositionIn[i].xyz - newP.xyz;
		vec3 b = gl_PositionIn[(i+1)%3].xyz - newP.xyz;
        ...
        gl_Position    = gl_ProjectionMatrix * newP;
        EmitVertex();
        EndPrimitive();
	}
}
